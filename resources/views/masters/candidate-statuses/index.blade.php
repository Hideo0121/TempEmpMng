...
@section('pageTitle', 'ステータスマスタ')
@section('pageDescription', '候補者の進捗ステータス名称・カラー・表示順を管理します。')

@section('content')
    <section class="rounded-3xl bg-white p-6 shadow-md">
        <header class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-lg font-semibold text-slate-900">ステータス一覧</h2>
                <p class="text-sm text-slate-500">表示順の昇順で表示しています。カラーコードは16進数6桁で設定します。</p>
            </div>
            <a href="{{ route('masters.candidate-statuses.create') }}" class="inline-flex items-center justify-center rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-500">新規追加</a>
        </header>
        ...
        <div class="mt-6 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex flex-col gap-3 md:flex-row md:items-center">
                <a
                    href="{{ route('masters.candidate-statuses.export') }}"
                ...
                >CSVダウンロード</a>
                <form
                    action="{{ route('masters.candidate-statuses.import') }}"
                    method="POST"
        ...
        <div class="mt-6 overflow-x-auto">
            <table class="min-w-full table-auto divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                    <tr>
                        <th class="px-4 py-2 text-left whitespace-nowrap">コード</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse ($statuses as $status)
                        <tr class="transition hover:bg-slate-50">
                            <td class="px-4 py-2 align-middle whitespace-nowrap font-mono text-xs text-slate-600">{{ $status->code }}</td>
                            <td class="px-4 py-2 align-middle whitespace-nowrap">
                                <span class="font-semibold text-slate-900 {{ $status->is_active ? '' : 'text-slate-400' }}">{{ $status->label }}</span>
                            </td>
                            <td class="px-4 py-2 align-middle whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <span class="inline-flex h-6 w-6 rounded-full border border-slate-200" style="background-color: {{ $status->color_code }}"></span>
                                    <span class="font-mono text-xs text-slate-600">{{ $status->color_code }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-2 align-middle whitespace-nowrap text-slate-700">{{ $status->sort_order }}</td>
                            <td class="px-4 py-2 align-middle whitespace-nowrap">
                                @if ($status->is_active)
        ...
        <footer class="mt-6 flex flex-col gap-3 text-sm text-slate-500 md:flex-row md:items-center md:justify-between">
            <span>
                全 {{ number_format($statuses->total()) }} 件中
                {{ $statuses->firstItem() }}〜{{ $statuses->lastItem() }} 件を表示
            </span>
